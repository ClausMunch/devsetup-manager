...
export async function startToolProcess(tool, execPath, args = []) {
    ...
    if (!await exists(resolved)) {
        ...
        async function findExecutable(dir, depth = 0) {
            if (depth > 3)
                return undefined;
            try {
                ...
                for (const e of entries) {
                    ...
                    if (e.isFile()) {
                        if (e.name.toLowerCase() === (tool + ext).toLowerCase())
                            return p;
...
export async function stopToolProcess(tool) {
...
export async function getToolLogs(tool) {
    ...
    try {
        ...
        let fileContents = '';
        try {
            fileContents = await fs.readFile(file, 'utf-8');
        }
        catch (e) { /* ignore */ }
...
export function isRunning(tool) {
    return !!procs[tool];
}
