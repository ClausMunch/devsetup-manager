...
app.whenReady().then(async () => {
    ...
    ipcMain.handle('install-tool', async (_e, name, version, downloadUrl, checksum) => {
        console.log('Install requested', name, version, downloadUrl);
        try {
            installProgress[name] = 0;
            // call installer with progress callback
            const saved = await installer(name, version, downloadUrl, checksum, (p) => { installProgress[name] = p; });
    ...
    ipcMain.handle('get-install-progress', async (_e, name) => {
        return installProgress[name] || 0;
    });
    ipcMain.handle('start-tool', async (_e, name) => {
    ...
    ipcMain.handle('stop-tool', async (_e, name) => {
        ...
        try {
            await appManager.stopToolProcess(name);
            toolStatus[name] = 'stopped';
            return { ok: true };
        }
        catch (e) {
    ...
    ipcMain.handle('get-nginx-sites', async () => {
        try {
            ...
            console.error('get-nginx-sites failed', e);
            return { webDir: '', folders: [] };
        }
    });
    ipcMain.handle('set-nginx-sites', async (_e, cfg) => {
        try {
            const fsp = await import('fs/promises');
                ...
                if (process.platform === 'win32') {
                    ...
                    try {
                        ...
                        const names = Array.from(new Set([
                            ...
                            ...(cfg.folders || [])
                        ].filter(Boolean).map((p) => path.basename(String(p)).replace(/[^a-zA-Z0-9_-]/g, '').toLowerCase())));
                        if (names.length > 0) {
    ...
    ipcMain.handle('remove-devsetup-hosts', async () => {
        try {
            if (process.platform !== 'win32')
                return { ok: true, info: 'Not Windows, nothing to do' };
            const fsp = await import('fs/promises');
            const hostsPath = path.join(process.env['SystemRoot'] || 'C:\\Windows', 'System32', 'drivers', 'etc', 'hosts');
            let hostsRaw = await fsp.readFile(hostsPath, 'utf-8');
            const startMarker = '# devsetup-manager hosts START';
            ...
            const sIdx = hostsRaw.indexOf(startMarker);
            if (sIdx === -1)
                return { ok: true, info: 'No devsetup-manager section found' };
            const eIdx = hostsRaw.indexOf(endMarker, sIdx);
            let newHosts = hostsRaw;
            if (eIdx !== -1)
                newHosts = hostsRaw.slice(0, sIdx) + hostsRaw.slice(eIdx + endMarker.length);
            else
                newHosts = hostsRaw.slice(0, sIdx);
            await fsp.writeFile(hostsPath, newHosts, 'utf-8');
